 ...
 */
    function forEach(obj, iterator, context) {
        var key, length;
        if (obj) if (isFunction(obj)) for (key in obj) // Need to check if hasOwnProperty exists,
        // as on IE8 the result of querySelectorAll is an object without a hasOwnProperty function
        "prototype" == key || "length" == key || "name" == key || obj.hasOwnProperty && !obj.hasOwnProperty(key) || iterator.call(context, obj[key], key, obj); else if (isArray(obj) || isArrayLike(obj)) {
 ...
 */
    function bootstrap(element, modules, config) {
        ...
        var doBootstrap = function() {
            ...
            } ]), config.debugInfoEnabled && // Pushing so that this overrides `debugInfoEnabled` setting defined in user's `modules`.
 ...
 */
    ...
    function bindJQuery() {
        ...
        bindJQueryFired || (// bind to jQuery if present;
        ...
        }) : jqLite = JQLite, angular.element = jqLite, // Prevent double-proxying.
        bindJQueryFired = !0);
    }
    /**
 ...
 */
    ...
    function jqLiteBuildFragment(html, context) {
        ...
        if (jqLiteIsTextNode(html)) // Convert non-html into a text node
        nodes.push(context.createTextNode(html)); else {
            for (// Convert html into DOM nodes
            tmp = tmp || fragment.appendChild(context.createElement("div")), tag = (TAG_NAME_REGEXP.exec(html) || [ "", "" ])[1].toLowerCase(), 
    ...
    function jqLiteOff(element, type, fn, unsupported) {
        ...
        if (handle) //no listeners registered
    ...
    function jqLiteAddNodes(root, elements) {
        ...
        if (elements.nodeType) root[root.length++] = elements; else {
    ...
    function jqLiteDocumentLoaded(action, win) {
        win = win || window, "complete" === win.document.readyState ? // Force the action to be run async for consistent behaviour
        // from the action's point of view
        // i.e. it will definitely not be in a $apply
        win.setTimeout(action) : // No need to unbind this handler as load is only ever called once
 ...
 */
   ...
   */
        self.url = function(url, replace, state) {
            // setter
            if (// In modern browsers `history.state` is `null` by default; treating it separately
            // from `undefined` would cause `$browser.url('/foo')` to change `history.state`
            // to undefined via `pushState`. Instead, let's change `undefined` to `null` here.
            isUndefined(state) && (state = null), // Android Browser BFCache causes location, history reference to become stale.
            location !== window.location && (location = window.location), history !== window.history && (history = window.history), 
            url) {
   ...
   */
        self.cookies = function(name, value) {
            ...
            if (!name) {
                ...
                index = cookie.indexOf("="), index > 0 && (//ignore nameless cookies
    ...
    function $CacheFactoryProvider() {
       ...
       */
                function link(nextEntry, prevEntry) {
                    nextEntry != prevEntry && (nextEntry && (nextEntry.p = prevEntry), //p stands for previous, 'prev' didn't minify
   ...
   */
        ...
        this.debugInfoEnabled = function(enabled) {
            ...
            function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
                ...
                return function(scope, cloneConnectFn, options) {
                    ...
                    var $linkNode;
                    if (// When using a directive with replace:true and templateUrl the $compileNodes
     ...
     */
            function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
                ...
                nodeLinkFnFound = nodeLinkFnFound || nodeLinkFn), //use the previous context only for the first element in the virtual group
                ...
                switch (nodeType) {
                  case NODE_TYPE_ELEMENT:
                    ...
                    for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; jj > j; j++) {
                        ...
                        attr = nAttrs[j], name = attr.name, value = trim(attr.value), // support ngAttr attribute binding
                        ngAttrName = directiveNormalize(name), (isNgAttr = NG_ATTR_BINDING.test(ngAttrName)) && (name = name.replace(PREFIX_REGEXP, "").substr(8).replace(/_(.)/g, function(match, letter) {
                    ...
                    if (// use class as directive
                    className = node.className, isObject(className) && (// Maybe SVGAnimatedString
                    className = className.animVal), isString(className) && "" !== className) for (;match = CLASS_DIRECTIVE_REGEXP.exec(className); ) nName = directiveNormalize(match[2]), 
     ...
     */
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    ...
                    boundTranscludeFn && (// track `boundTranscludeFn` so it can be unwrapped if `transcludeFn`
                    // is later passed as `parentBoundTranscludeFn` to `publicLinkFn`
                    transcludeFn = controllersBoundTransclude, transcludeFn.$$boundTransclude = boundTranscludeFn), 
                    controllerDirectives && (// TODO: merge `controllers` and `elementControllers` into single object.
                    controllers = {}, elementControllers = {}, forEach(controllerDirectives, function(directive) {
                        ...
                        forEach(isolateScope.$$isolateBindings = newIsolateScopeDirective.$$isolateBindings, function(definition, scopeName) {
                            var // @, =, or &
                            lastValue, parentGet, parentSet, compare, attrName = definition.attrName, optional = definition.optional, mode = definition.mode;
                            switch (mode) {
                              case "@":
                ...
                for (var newScopeDirective, controllers, directive, directiveName, $template, linkFn, directiveValue, terminalPriority = -Number.MAX_VALUE, controllerDirectives = previousCompileContext.controllerDirectives, newIsolateScopeDirective = previousCompileContext.newIsolateScopeDirective, templateDirective = previousCompileContext.templateDirective, nonTlbTranscludeDirective = previousCompileContext.nonTlbTranscludeDirective, hasTranscludeDirective = !1, hasTemplate = !1, hasElementTranscludeDirective = previousCompileContext.hasElementTranscludeDirective, $compileNode = templateAttrs.$$element = jqLite(compileNode), replaceDirective = originalReplaceDirective, childTranscludeFn = transcludeFn, i = 0, ii = directives.length; ii > i; i++) {
                    ...
                    if (// collect multiblock sections
                    attrStart && ($compileNode = groupScan(compileNode, attrStart, attrEnd)), $template = undefined, 
                    ...
                    directive.templateUrl || (isObject(directiveValue) ? (// This directive is trying to add an isolated scope.
                    // Check that there is no scope of any kind already
                    assertNoDuplicate("new/isolated scope", newIsolateScopeDirective || newScopeDirective, directive, $compileNode), 
                    newIsolateScopeDirective = directive) : // This directive is trying to add a child scope.
                    ...
                    forEach($rootElement, function(node, i) {
                        ...
                        if (!scope.$$destroyed) {
                            if (beforeTemplateLinkNode !== beforeTemplateCompileNode) {
                                ...
                                previousCompileContext.hasElementTranscludeDirective && origAsyncDirective.replace || (// it was cloned therefore we have to clone as well.
                        ...
                        compile: function() {
                            return {
                                pre: function(scope, element, attr) {
                                    ...
                                    newValue !== value && (// we need to interpolate again since the attribute value has been updated
                                    // (e.g. by another directive's compile function)
                                    // ensure unset/empty values make interpolateFn falsy
                                    interpolateFn = newValue && $interpolate(newValue, !0, trustedContext, allOrNothing), 
                                    value = newValue), // if attribute was updated so that there is no interpolation going on we don't want to
                                    // register any observers
                                    interpolateFn && (// initialize attr object so that it's ready in case we need the value for isolate
                                    // scope initialization, otherwise the value would not be available from isolate
                                    // directive's linking fn during linking phase
                                    attr[name] = interpolateFn(scope), ($$observers[name] || ($$observers[name] = [])).$$inter = !0, 
     ...
     */
            function replaceWith($rootElement, elementsToRemove, newNode) {
                ...
                fragment.appendChild(firstElementToRemove), // Copy over user data (that includes Angular's $scope etc.). Don't copy private
                ...
                jQuery ? (// jQuery 2.x doesn't expose the data storage. Use jQuery.cleanData to clean up after
                ...
                skipDestroyOnNextJQueryCleanData = !0, jQuery.cleanData([ firstElementToRemove ])) : delete jqLite.cache[firstElementToRemove[jqLite.expando]];
                for (var k = 1, kk = elementsToRemove.length; kk > k; k++) {
                    var element = elementsToRemove[k];
                    jqLite(element).remove(), // must do this way to clean up expando
                    fragment.appendChild(element), delete elementsToRemove[k];
                }
       ...
       */
                $set: function(key, value, writeAttr, attrName) {
                    ...
                    observer = aliasedKey), this[key] = value, // translate normalized key to actual key
                    attrName ? this.$attr[key] = attrName : (attrName = this.$attr[key], attrName || (this.$attr[key] = attrName = snake_case(key, "-"))), 
                    nodeName = nodeName_(this.$$element), "a" === nodeName && "href" === key || "img" === nodeName && "src" === key) // sanitize a[href] and img[src] values
                    this[key] = value = $$sanitizeUri(value, "src" === key); else if ("img" === nodeName && "srcset" === key) {
                        for (var result = "", trimmedSrcset = trim(value), srcPattern = /(\s+\d+x\s*,|\s+\d+w\s*,|\s+,|,\s+)/, pattern = /\s/.test(trimmedSrcset) ? srcPattern : /(,)/, rawUris = trimmedSrcset.split(pattern), nbrUrisWith2parts = Math.floor(rawUris.length / 2), i = 0; nbrUrisWith2parts > i; i++) {
                            var innerIdx = 2 * i;
                            // sanitize the uri
                            result += $$sanitizeUri(trim(rawUris[innerIdx]), !0), // add the descriptor
                            result += " " + trim(rawUris[innerIdx + 1]);
                        }
                        // split the last item into uri and descriptor
                        var lastTuple = trim(rawUris[2 * i]).split(/\s/);
                        // sanitize the last uri
                        result += $$sanitizeUri(trim(lastTuple[0]), !0), // and add the last descriptor if any
                        2 === lastTuple.length && (result += " " + trim(lastTuple[1])), this[key] = value = result;
                    }
       ...
       */
                $observe: function(key, fn) {
                    ...
                    return listeners.push(fn), $rootScope.$evalAsync(function() {
                        !listeners.$$inter && attrs.hasOwnProperty(key) && // no one registered attribute interpolation function, so lets call it manually
                        fn(attrs[key]);
                    }), function() {
     ...
     */
       ...
       */
                function done(status, response, headersString, statusText) {
                    ...
                    cache && (isSuccess(status) ? cache.put(url, [ status, response, parseHeaders(headersString), statusText ]) : // remove promise from the cache
                    cache.remove(url)), useApplyAsync ? $rootScope.$applyAsync(resolveHttpPromise) : (resolveHttpPromise(), 
       ...
       */
                ...
                cache && (cachedResp = cache.get(url), isDefined(cachedResp) ? isPromiseLike(cachedResp) ? // cached request has already been sent, but there is no response yet
                cachedResp.then(resolvePromiseWithResult, resolvePromiseWithResult) : // serving from cache
                isArray(cachedResp) ? resolvePromise(cachedResp[1], cachedResp[0], shallowCopy(cachedResp[2]), cachedResp[3]) : resolvePromise(cachedResp, 200, {}, "OK") : // put the promise for the non-transformed response into cache as a placeholder
                cache.put(url, promise)), isUndefined(cachedResp)) {
 ...
 */
   ...
   */
        this.$$compose = function() {
            ...
            if (relHref && "#" === relHref[0]) // special case for links to hash fragments:
   ...
   */
     ...
     */
      ...
      */
            ...
            "#" === withoutBaseUrl.charAt(0) ? (// The rest of the url starts with a hash so we have
            // got either a hashbang path or a plain hash fragment
            withoutHashUrl = beginsWith(hashPrefix, withoutBaseUrl), isUndefined(withoutHashUrl) && (// There was no hashbang prefix so we just have a hash fragment
            withoutHashUrl = withoutBaseUrl)) : // There was no hashbang path nor hash fragment:
            // If we are in HTML5 mode we use what is left as the path;
            // Otherwise we ignore what is left
            withoutHashUrl = this.$$html5 ? withoutBaseUrl : "", parseAppUrl(withoutHashUrl, this), 
 ...
 */
    function LocationHashbangInHtml5Url(appBase, hashPrefix) {
        ...
        this.$$parseLinkUrl = function(url, relHref) {
            ...
            var search = toKeyValue(this.$$search), hash = this.$$hash ? "#" + encodeUriSegment(this.$$hash) : "";
            this.$$url = encodePath(this.$$path) + (search ? "?" + search : "") + hash, // include hashPrefix in $$absUrl when $$url is empty so IE8 & 9 do not reload page because of removal of '#'
 ...
 */
   ...
   */
        this.$get = [ "$rootScope", "$browser", "$sniffer", "$rootElement", "$window", function($rootScope, $browser, $sniffer, $rootElement, $window) {
            ...
            var $location, LocationMode, appBase, baseHref = $browser.baseHref(), // if base[href] is undefined, it defaults to ''
            ...
            $rootElement.on("click", function(event) {
                ...
                if (html5Mode.rewriteLinks && !event.ctrlKey && !event.metaKey && !event.shiftKey && 2 != event.which && 2 != event.button) {
                    ...
                    for (var elm = jqLite(event.target); "a" !== nodeName_(elm[0]); ) // ignore rewriting if no A tag (reached root element, or no parent - removed from document)
                    if (elm[0] === $rootElement[0] || !(elm = elm.parent())[0]) return;
                    var absHref = elm.prop("href"), relHref = elm.attr("href") || elm.attr("xlink:href");
                    isObject(absHref) && "[object SVGAnimatedString]" === absHref.toString() && (// SVGAnimatedString.animVal should be identical to SVGAnimatedString.baseVal, unless during
                    // an animation.
                    absHref = urlResolve(absHref.animVal).href), // Ignore when url is started with javascript: or mailto:
                    IGNORE_URI_REGEXP.test(absHref) || !absHref || elm.attr("target") || event.isDefaultPrevented() || $location.$$parseLinkUrl(absHref, relHref) && (// We do a preventDefault for all urls that are part of the angular application,
                    // in html5mode and also without, so that we are able to abort navigation without
                    // getting double entries in the location history.
                    event.preventDefault(), // update location manually
                    $location.absUrl() != $browser.url() && ($rootScope.$apply(), // hack to work around FF6 bug 684208 when scenario runner clicks on links
   ...
   */
    ...
    function ensureSafeObject(obj, fullExpression) {
        ...
        if (obj) {
            ...
            if (// isWindow(obj)
            obj.window === obj) throw $parseMinErr("isecwindow", "Referencing the Window in Angular expressions is disallowed! Expression: {0}", fullExpression);
            if (// isElement(obj)
            obj.children && (obj.nodeName || obj.prop && obj.attr && obj.find)) throw $parseMinErr("isecdom", "Referencing DOM nodes in Angular expressions is disallowed! Expression: {0}", fullExpression);
            if (// block Object so that we can't get hold of dangerous Object.* methods
            obj === Object) throw $parseMinErr("isecobj", "Referencing Object in Angular expressions is disallowed! Expression: {0}", fullExpression);
        }
        return obj;
    }
 ...
 */
    ...
    function getterFn(path, options, fullExp) {
        ...
        if (options.csp) fn = 6 > pathKeysLength ? cspSafeGetterFn(pathKeys[0], pathKeys[1], pathKeys[2], pathKeys[3], pathKeys[4], fullExp, expensiveChecks) : function(scope, locals) {
            ...
            scope = val; while (pathKeysLength > i);
            return val;
        }; else {
            var code = "";
        ...
        this.$get = [ "$filter", "$sniffer", function($filter, $sniffer) {
            ...
            function collectExpressionInputs(inputs, list) {
                for (var i = 0, ii = inputs.length; ii > i; i++) {
                    var input = inputs[i];
                    input.constant || (input.inputs ? collectExpressionInputs(input.inputs, list) : -1 === list.indexOf(input) && // TODO(perf) can we do better?
                    list.push(input));
                }
                return list;
            }
                ...
                switch (typeof exp) {
                  case "string":
                    ...
                    if (parsedExpression = cache[cacheKey], !parsedExpression) {
                        ...
                        parsedExpression = parser.parse(exp), parsedExpression.constant ? parsedExpression.$$watchDelegate = constantWatchDelegate : oneTime ? (//oneTime is not part of the exp passed to the Parser so we may have to
                        //wrap the parsedExpression before adding a $$watchDelegate
                        parsedExpression = wrapSharedExpression(parsedExpression), parsedExpression.$$watchDelegate = parsedExpression.literal ? oneTimeLiteralWatchDelegate : oneTimeWatchDelegate) : parsedExpression.inputs && (parsedExpression.$$watchDelegate = inputsWatchDelegate), 
 ...
 */
    function qFactory(nextTick, exceptionHandler) {
        ...
        function Deferred() {
            this.promise = new Promise(), //Necessary to support unbound execution :/
            this.resolve = simpleBind(this, this.resolve), this.reject = simpleBind(this, this.reject), 
   ...
   */
        var reject = function(reason) {
            ...
            if (!(this instanceof Q)) // More useful when $Q is the Promise itself.
      ...
      */
                    ...
                    function $watchCollectionInterceptor(_value) {
                        ...
                        if (!isUndefined(newValue)) {
                            if (isObject(newValue)) if (isArrayLike(newValue)) {
                                oldValue !== internalArray && (// we are transitioning from something which was not an array into array.
                                oldValue = internalArray, oldLength = oldValue.length = 0, changeDetected++), newLength = newValue.length, 
                                oldLength !== newLength && (// if lengths do not match we need to trigger change notification
                                ...
                                oldValue !== internalObject && (// we are transitioning from something which was not an object into object.
       ...
       */
                $digest: function() {
                    ...
                    $browser.$$checkUrlChange(), this === $rootScope && null !== applyAsyncId && (// If this is the root scope, and $applyAsync has scheduled a deferred $apply(), then
                    // cancel the scheduled $apply and flush the queue of expressions to be evaluated.
                    $browser.defer.cancel(applyAsyncId), flushApplyAsync()), lastDirtyWatch = null;
                    do {
                        for (// "while dirty" loop
                        dirty = !1, current = target; asyncQueue.length; ) {
                            try {
       ...
       */
                $emit: function(name) {
                    ...
                    do {
                        ...
                        i = 0, length = namedListeners.length; length > i; i++) // if listeners were deregistered, defragment the array
       ...
       */
                $broadcast: function(name) {
                    ...
                    for (var listeners, i, length, listenerArgs = concat([ event ], arguments, 1); current = next; ) {
                        ...
                        i = 0, length = listeners.length; length > i; i++) // if listeners were deregistered, defragment the array
                        if (listeners[i]) try {
   ...
   */
        this.resourceUrlBlacklist = function(value) {
            ...
            function isResourceUrlAllowedByPolicy(url) {
                ...
                for (i = 0, n = resourceUrlBlacklist.length; n > i; i++) if (matchUrl(resourceUrlBlacklist[i], parsedUrl)) {
                    allowed = !1;
                    break;
                }
                return allowed;
 ...
 */
    ...
    function deepCompare(actual, expected, comparator, matchAgainstAnyProp, dontMatchWholeObject) {
        ...
        if (isArray(actual)) // In case `actual` is an array, consider it a match
        ...
        return function(array, sortPredicate, reverseOrder) {
            ...
            0 === sortPredicate.length && (sortPredicate = [ "+" ]), sortPredicate = sortPredicate.map(function(predicate) {
                ...
                if (isString(predicate)) {
                    ...
                    predicate = predicate.substring(1)), "" === predicate) // Effectively no predicate was passed so we compare identity
   ...
   */
    ...
    function createDateParser(regexp, mapping) {
        return function(iso, date) {
            ...
            if (isString(iso)) {
                if (// When a date is JSON'ified to wraps itself inside of an extra
 ...
 */
    ...
    slice = [].slice, splice = [].splice, push = [].push, toString = Object.prototype.toString, ngMinErr = minErr("ng"), /** @name angular */
 ...
 */
        ...
        JQLite.prototype[name] = function(arg1, arg2) {
            ...
            if (fn !== jqLiteEmpty && (2 == fn.length && fn !== jqLiteHasClass && fn !== jqLiteController ? arg1 : arg2) === undefined) {
                if (isObject(arg1)) {
                    ...
                    for (i = 0; nodeCount > i; i++) if (fn === jqLiteData) // data() takes the whole object in jQuery
    ...
    forEach({
        ...
        on: function jqLiteOn(element, type, fn, unsupported) {
            ...
            if (jqLiteAcceptsData(element)) {
                ...
                for (// http://jsperf.com/string-indexof-vs-split
                var types = type.indexOf(" ") >= 0 ? type.split(" ") : [ type ], i = types.length; i--; ) {
                    type = types[i];
                    var eventFns = events[type];
                    eventFns || (events[type] = [], "mouseenter" === type || "mouseleave" === type ? // Refer to jQuery's implementation of mouseenter & mouseleave
        ...
        triggerHandler: function(element, event, extraParameters) {
            ...
            eventFns && (// Create a dummy event to pass to the handlers
            dummyEvent = {
                preventDefault: function() {
            ...
            event.type && (dummyEvent = extend(dummyEvent, event)), // Copy event handlers in case event handlers array is modified during execution.
   ...
   */
        JQLite.prototype[name] = function(arg1, arg2, arg3) {
            ...
            isDefined(value) && (// any function which returns a value needs to be wrapped
 ...
 */
    ...
    Parser.ZERO = extend(function() {
        ...
        statements: function() {
            ...
            !this.expect(";")) // optimize for the common case where there is only one statement.
        ...
        filter: function(inputFn) {
            ...
            if (this.peek(":")) // we can safely reuse the array
            for (argsFn = [], args = []; this.expect(":"); ) argsFn.push(this.expression());
        ...
        arrayDeclaration: function() {
            ...
            if ("]" !== this.peekToken().text) do {
                if (this.peek("]")) // Support trailing commas per ES5.1.
                break;
                    ...
                    return {
                        pre: function(scope, formElement, attr, controller) {
                            ...
                            if (!("action" in attr)) {
                                ...
                                addEventListenerFn(formElement[0], "submit", handleFormSubmission), // unregister the preventDefault listener so that we don't not leak memory but in a
        ...
        ngEventDirectives[directiveName] = [ "$parse", "$rootScope", function($parse, $rootScope) {
            return {
                ...
                compile: function($element, attr) {
                    ...
                    var fn = $parse(attr[directiveName], /* interceptorFn */ null, /* expensiveChecks */ !0);
 ...
 */
    var ngIfDirective = [ "$animate", function($animate) {
            ...
            compile: function(element, attr) {
                ...
                return function(scope, $element, $attr, ctrl, $transclude) {
                    ...
                    scope.$watch($sce.parseAsResourceUrl(srcExp), function(src) {
                        ...
                        src ? (//set the 2nd param to true to ignore the template request error so that the inner
        ...
        this.$dirty = !1, this.$valid = !0, this.$invalid = !1, this.$error = {}, // keep invalid keys here
        ...
        this.$validate = function() {
            ...
            if (!isNumber(ctrl.$modelValue) || !isNaN(ctrl.$modelValue)) {
                ...
                ctrl.$$runValidators(modelValue, viewValue, function(allValid) {
                    ...
                    allowInvalid || prevValid === allValid || (// Note: Don't check ctrl.$valid here, as we could have
                    // external validators (e.g. calculated on the server),
                    // that just call $setValidity and need the model value
                    // to calculate their validity.
                    ctrl.$modelValue = allValid ? modelValue : undefined, ctrl.$modelValue !== prevModelValue && ctrl.$$writeModelToScope());
                });
            }
   ...
   */
        this.$commitViewValue = function() {
            ...
            $timeout.cancel(pendingDebounce), // If the view value has not changed then we should just exit, except in the case where there is
            ...
            isNumber(ctrl.$modelValue) && isNaN(ctrl.$modelValue) && (// ctrl.$modelValue has not been touched yet...
            ...
            writeToModelIfNeeded()), // Pass the $$lastCommittedViewValue here, because the cached viewValue might be out of date.
            // This can happen if e.g. $setViewValue is called from inside a parser
            ctrl.$$runValidators(modelValue, ctrl.$$lastCommittedViewValue, function(allValid) {
                allowInvalid || (// Note: Don't check ctrl.$valid here, as we could have
                // external validators (e.g. calculated on the server),
                // that just call $setValidity and need the model value
                // to calculate their validity.
                ctrl.$modelValue = allValid ? modelValue : undefined, writeToModelIfNeeded());
            });
   ...
   */
            ...
            compile: function(element) {
                ...
                return element.addClass(PRISTINE_CLASS).addClass(UNTOUCHED_CLASS).addClass(VALID_CLASS), 
                {
                    pre: function(scope, element, attr, ctrls) {
                        ...
                        modelCtrl.$$setOptions(ctrls[2] && ctrls[2].$options), // notify others, especially parent forms
        ...
        return {
            ...
            controller: [ "$scope", "$attrs", function($scope, $attrs) {
                var that = this;
                this.$options = $scope.$eval($attrs.ngModelOptions), // Allow adding/overriding bound events
                this.$options.updateOn !== undefined ? (this.$options.updateOnDefault = !1, // extract "default" pseudo-event from list of events that can trigger a model update
        ...
        return {
            ...
            link: function(scope, element, attr) {
                ...
                var lastCount, numberExp = attr.count, whenExp = attr.$attr.when && element.attr(attr.$attr.when), // we have {{}} in attrs
                offset = attr.offset || 0, whens = scope.$eval(whenExp) || {}, whensExpFns = {}, startSymbol = $interpolate.startSymbol(), endSymbol = $interpolate.endSymbol(), braceReplacement = startSymbol + numberExp + "-" + offset + endSymbol, watchRemover = angular.noop;
                forEach(attr, function(expression, attributeName) {
                    ...
                    countIsNaN || count in whens || (// If an explicit number rule such as 1, 2, 3... is defined, just use it.
                    ...
                    count === lastCount || countIsNaN && isNaN(lastCount) || (watchRemover(), watchRemover = scope.$watch(whensExpFns[count], updateElementText), 
                    lastCount = count);
                });
            }
        };
            ...
            compile: function($element, $attr) {
                ...
                return trackByExp ? trackByExpGetter = $parse(trackByExp) : (trackByIdArrayFn = function(key, value) {
                    ...
                    $scope.$watchCollection(rhs, function(collection) {
                        ...
                        nextBlockOrder, elementsToRemove, previousNode = $element[0], // Same as lastBlockMap but it has the current state. It will become the
                        ...
                        trackByIdFn = trackByIdExpFn || trackByIdArrayFn; else {
                            trackByIdFn = trackByIdExpFn || trackByIdObjFn, // if object, extract keys, sort them and use to determine order of iteration over obj props
                            collectionKeys = [];
                        ...
                        value = collection[key], trackById = trackByIdFn(key, value, index), lastBlockMap[trackById]) // found previously seen block
                        block = lastBlockMap[trackById], delete lastBlockMap[trackById], nextBlockMap[trackById] = block, 
                        nextBlockOrder[index] = block; else {
                            if (nextBlockMap[trackById]) // if collision detected. restore lastBlockMap and throw an error
                        ...
                        for (var blockKey in lastBlockMap) {
                            ...
                            $animate.leave(elementsToRemove), elementsToRemove[0].parentNode) // if the element was not removed yet because of pending animation, mark it as deleted
                        ...
                        value = collection[key], block = nextBlockOrder[index], block.scope) {
                            ...
                            getBlockStart(block) != nextNode && // existing item which got moved
                            $animate.move(getBlockNodes(block.clone), null, jqLite(previousNode)), previousNode = getBlockEnd(block), 
                        ...
                        $transclude(function(clone, scope) {
                            ...
                            clone[clone.length++] = endNode, // TODO(perf): support naked previousNode in `enter` to avoid creation of jqLite wrapper?
        ...
        return {
            ...
            controller: [ "$element", "$scope", "$attrs", function($element, $scope, $attrs) {
                ...
                self.databound = $attrs.ngModel, self.init = function(ngModelCtrl_, nullOption_, unknownOption_) {
                    ...
                    unknownOption.parent() && unknownOption.remove()), // Workaround for https://code.google.com/p/chromium/issues/detail?id=381459
                ...
                function setupAsOptions(scope, selectElement, ctrl) {
                    ...
                    function createIsSelectedFn(viewValue) {
                        ...
                        if (multiple) if (trackFn && isArray(viewValue)) {
                            ...
                            for (var trackIndex = 0; trackIndex < viewValue.length; trackIndex++) // tracking by key
        ...
        */
                    ...
                    function render() {
                        ...
                        trackFn && (trackKeysCache[optionId] = key), optionGroup.push({
                        ...
                        for (multiple || (nullOption || null === viewValue ? // insert null option if we have a placeholder, or the model is null
                        optionGroups[""].unshift({
                            id: "",
                            label: "",
                            selected: !anySelected
                        }) : anySelected || // option could not be found, we have to insert the undefined item
                        optionGroups[""].unshift({
                            id: "?",
                            label: "",
                            selected: !0
                            ...
                            for (// current option group name or '' if no group
                            optionGroupName = optionGroupNames[groupIndex], // list of options for that group. (first item has the parent)
                            optionGroup = optionGroups[optionGroupName], optionGroupsCache.length <= groupIndex ? (// we need to grow the optionGroups
                            existingParent = {
                            ...
                            lastElement = existingOption.element, existingOption.label !== option.label && (updateLabelMap(labelMap, existingOption.label, !1), 
                            ...
                            lastElement.prop("selected", existingOption.selected))) : (// grow elements
                            // if it's a null option
                            "" === option.id && nullOption ? // put back the pre-compiled element
                            ...
                            index++; existingOptions.length > index; ) option = existingOptions.pop(), updateLabelMap(labelMap, option.label, !1), 
                            option.element.remove();
                        }
                        // remove any excessive OPTGROUPs from select
                        for (;optionGroupsCache.length > groupIndex; ) {
                            for (// remove all the labels in the option group
                            optionGroup = optionGroupsCache.pop(), index = 1; index < optionGroup.length; ++index) updateLabelMap(labelMap, optionGroup[index].label, !1);
                    ...
                    selectElement.empty(), selectElement.on("change", selectionChanged), ctrl.$render = render, 
                ...
                if (ctrls[1]) {
                    ...
                    emptyOption, selectCtrl = ctrls[0], ngModelCtrl = ctrls[1], multiple = attr.multiple, optionsExp = attr.ngOptions, nullOption = !1, renderScheduled = !1, // we can't just jqLite('<option>') since jqLite is not smart enough
                    // to create it in <select> and IE barfs otherwise.
                    optionTemplate = jqLite(document.createElement("option")), optGroupTemplate = jqLite(document.createElement("optgroup")), unknownOption = optionTemplate.clone(), i = 0, children = element.children(), ii = children.length; ii > i; i++) if ("" === children[i].value) {
                        emptyOption = nullOption = children.eq(i);
                        break;
                    }
                    selectCtrl.init(ngModelCtrl, nullOption, unknownOption), // required validator
        ...
        return {
            ...
            link: function(scope, elm, attr, ctrl) {
                ctrl && (attr.required = !0, // force truthy in case we are on non input element
       ...
       */
        function push() {
            ...
            if (updated) {
                ...
                for (name in cookies) cookies[name] !== browserCookies[name] && (//delete or reset all cookies that the browser dropped from $cookies
                isUndefined(browserCookies[name]) ? delete cookies[name] : cookies[name] = browserCookies[name], 
                updated = !0);
            }
        }
        ...
        return $browser.addPollFn(function() {
            ...
            lastBrowserCookies != currentCookies && (//relies on browser.cookies() impl
            lastBrowserCookies = currentCookies, copy(currentCookies, lastCookies), copy(currentCookies, cookies), 
            runEval && $rootScope.$apply());
       ...
       */
            ...
            return Route.prototype = {
                setUrlParams: function(config, params, actionUrl) {
                    ...
                    self.defaults.stripTrailingSlashes && (url = url.replace(/\/+$/, "") || "/"), // then replace collapse `/.` if found in the last URL path segment before the query
                    // E.g. `http://url.com/id./format?q=x` becomes `http://url.com/id.format?q=x`
                    url = url.replace(/\/\.(?=\w+($|\?))/, "."), // replace escaped `/\.` with `/.`
                    config.url = url.replace(/\/\\\./, "/."), // set params - delegate param encoding to $http
 ...
 */
    ngViewFactory.$inject = [ "$route", "$anchorScroll", "$animate" ], // This directive is called during the $transclude call of the first `ngView` directive.
 ...
 */
    function htmlParser(html, handler) {
        ...
        function parseEndTag(tag, tagName) {
            var i, pos = 0;
            if (tagName = angular.lowercase(tagName)) // Find the closest opened tag of the same type
        ...
        for (stack.last = function() {
            ...
            if (text = "", chars = !0, // Make sure we're not in a script or style element
            stack.last() && specialElements[stack.last()] ? (// IE versions 9 and 10 do not understand the regex '[^]', so using a workaround with [\W\w].
            html = html.replace(new RegExp("([\\W\\w]*)<\\s*\\/\\s*" + stack.last() + "[^>]*>", "i"), function(all, text) {
                ...
                "";
            }), parseEndTag("", stack.last())) : (// Comment
            0 === html.indexOf("<!--") ? (// comments containing -- are not allowed unless they terminate the comment
            index = html.indexOf("--", 4), index >= 0 && html.lastIndexOf("-->", index) === index && (handler.comment && handler.comment(html.substring(4, index)), 
            ...
            match ? (// We only have a valid start-tag if there is a '>'.
            match[4] && (html = html.substring(match[0].length), match[0].replace(START_TAG_REGEXP, parseStartTag)), 
            chars = !1) : (// no ending tag found --- this piece should be encoded as an entity.
            text += "<", html = html.substring(1))), chars && (index = html.indexOf("<"), text += 0 > index ? html : html.substring(0, index), 
 ...
 */
    angular.module("ngSanitize").filter("linky", [ "$sanitize", function($sanitize) {
        ...
        return function(text, target) {
            ...
            if (!text) return text;
            for (var match, url, i, raw = text, html = []; match = raw.match(LINKY_URL_REGEXP); ) // We can not end in these as they are sometimes found at the end of the sentence
...
function() {
    ...
    SelectParser = function() {
        ...
        return SelectParser.prototype.add_node = function(child) {
            ...
            }), _ref = group.childNodes, _results = [], _i = 0, _len = _ref.length; _len > _i; _i++) option = _ref[_i], 
            _results.push(this.add_option(option, group_position, group.disabled));
            return _results;
        }, SelectParser.prototype.add_option = function(option, group_position, group_disabled) {
            return "OPTION" === option.nodeName.toUpperCase() ? ("" !== option.text ? (null != group_position && (this.parsed[group_position].children += 1), 
            this.parsed.push({
        ...
        for (parser = new SelectParser(), _ref = select.childNodes, _i = 0, _len = _ref.length; _len > _i; _i++) child = _ref[_i], 
        ...
        return AbstractChosen.prototype.set_default_values = function() {
            ...
            for (content = "", _ref = this.results_data, _i = 0, _len = _ref.length; _len > _i; _i++) data = _ref[_i], 
            content += data.group ? this.result_add_group(data) : this.result_add_option(data), 
            (null != options ? options.first : void 0) && (data.selected && this.is_multiple ? this.choice_build(data) : data.selected && !this.is_multiple && this.single_set_selected_text(data.text));
            return content;
            ...
            var group_el;
            return (group.search_match || group.group_match) && group.active_options > 0 ? (group_el = document.createElement("li"), 
            group_el.className = "group-result", group_el.innerHTML = group.search_text, this.outerHTML(group_el)) : "";
            ...
            for (_ref = this.results_data, _results = [], _i = 0, _len = _ref.length; _len > _i; _i++) result = _ref[_i], 
            _results.push(result.selected ? result.selected = !1 : void 0);
            return _results;
            ...
            regex = new RegExp(regexAnchor + escapedSearchText, "i"), zregex = new RegExp(escapedSearchText, "i"), 
            _ref = this.results_data, _i = 0, _len = _ref.length; _len > _i; _i++) option = _ref[_i], 
            option.search_match = !1, results_group = null, this.include_option_in_results(option) && (option.group && (option.group_match = !1, 
            ...
            results_group.active_options += 1), (!option.group || this.group_search) && (option.search_text = option.group ? option.label : option.html, 
            option.search_match = this.search_string_match(option.search_text, regex), option.search_match && !option.group && (results += 1), 
            option.search_match ? (searchText.length && (startpos = option.search_text.search(zregex), 
            ...
            return this.result_clear_highlight(), 1 > results && searchText.length ? (this.update_results_content(""), 
            ...
            parts.length)) for (_i = 0, _len = parts.length; _len > _i; _i++) if (part = parts[_i], 
            regex.test(part)) return !0;
        }, AbstractChosen.prototype.choices_count = function() {
            var option, _i, _len, _ref;
            if (null != this.selected_option_count) return this.selected_option_count;
            for (this.selected_option_count = 0, _ref = this.form_field.options, _i = 0, _len = _ref.length; _len > _i; _i++) option = _ref[_i], 
        ...
        return __extends(Chosen, _super), Chosen.prototype.setup = function() {
            ...
            this.container = $("<div />", container_props), this.container.html(this.is_multiple ? '<ul class="chosen-choices"><li class="search-field"><input type="text" value="' + this.default_text + '" class="default" autocomplete="off" style="width:25px;" /></li></ul><div class="chosen-drop"><ul class="chosen-results"></ul></div>' : '<a class="chosen-single chosen-default" tabindex="-1"><span>' + this.default_text + '</span><div><b></b></div></a><div class="chosen-drop"><div class="chosen-search"><input type="text" autocomplete="off" /></div><ul class="chosen-results"></ul></div>'), 
            ...
            return this.is_disabled || (evt && "mousedown" === evt.type && !this.results_showing && evt.preventDefault(), 
            null != evt && $(evt.target).hasClass("search-choice-close")) ? void 0 : (this.active_field ? this.is_multiple || !evt || $(evt.target)[0] !== this.selected_item[0] && !$(evt.target).parents("a.chosen-single").length || (evt.preventDefault(), 
            ...
            var delta;
            return evt.originalEvent && (delta = -evt.originalEvent.wheelDelta || evt.originalEvent.detail), 
            null != delta ? (evt.preventDefault(), "DOMMouseScroll" === evt.type && (delta = 40 * delta), 
            this.search_results.scrollTop(delta + this.search_results.scrollTop())) : void 0;
            ...
            if (el.length) {
                ...
                high_bottom = high_top + this.result_highlight.outerHeight(), high_bottom >= visible_bottom) return this.search_results.scrollTop(high_bottom - maxHeight > 0 ? high_bottom - maxHeight : 0);
            ...
            var ti;
            return this.form_field.tabIndex ? (ti = this.form_field.tabIndex, this.form_field.tabIndex = -1, 
            this.search_field[0].tabIndex = ti) : void 0;
        }, Chosen.prototype.set_label_behavior = function() {
            var _this = this;
            return this.form_field_label = this.form_field_jq.parents("label"), !this.form_field_label.length && this.form_field.id.length && (this.form_field_label = $("label[for='" + this.form_field.id + "']")), 
            this.form_field_label.length > 0 ? this.form_field_label.bind("click.chosen", function(evt) {
            ...
            var target;
            return target = $(evt.target).hasClass("active-result") ? $(evt.target) : $(evt.target).parents(".active-result").first(), 
            target.length ? (this.result_highlight = target, this.result_select(evt), this.search_field.focus()) : void 0;
        }, Chosen.prototype.search_results_mouseover = function(evt) {
            var target;
            return target = $(evt.target).hasClass("active-result") ? $(evt.target) : $(evt.target).parents(".active-result").first(), 
            target ? this.result_do_highlight(target) : void 0;
            ...
            return this.result_deselect(link[0].getAttribute("data-option-array-index")) ? (this.show_search_field_default(), 
            this.is_multiple && this.choices_count() > 0 && this.search_field.val().length < 1 && this.results_hide(), 
            link.parents("li").first().remove(), this.search_field_scale()) : void 0;
            ...
            var high, item;
            return this.result_highlight ? (high = this.result_highlight, this.result_clear_highlight(), 
            this.is_multiple && this.max_selected_options <= this.choices_count() ? (this.form_field_jq.trigger("chosen:maxselected", {
                chosen: this
            }), !1) : (this.is_multiple ? high.removeClass("active-result") : this.reset_single_select_options(), 
            item = this.results_data[high[0].getAttribute("data-option-array-index")], item.selected = !0, 
            this.form_field.options[item.options_index].selected = !0, this.selected_option_count = null, 
            this.is_multiple ? this.choice_build(item) : this.single_set_selected_text(item.text), 
            ...
            this.selected_item.removeClass("chosen-default")), this.selected_item.find("span").text(text);
        }, Chosen.prototype.result_deselect = function(pos) {
            var result_data;
            return result_data = this.results_data[pos], this.form_field.options[result_data.options_index].disabled ? !1 : (result_data.selected = !1, 
            ...
            return selected_results = this.is_multiple ? [] : this.search_results.find(".result-selected.active-result"), 
            do_high = selected_results.length ? selected_results.first() : this.search_results.find(".active-result").first(), 
            null != do_high ? this.result_do_highlight(do_high) : void 0;
            ...
            if (this.is_multiple) {
                for (h = 0, w = 0, style_block = "position:absolute; left: -1000px; top: -1000px; display:none;", 
                styles = [ "font-size", "font-style", "font-weight", "font-family", "line-height", "text-transform", "letter-spacing" ], 
                _i = 0, _len = styles.length; _len > _i; _i++) style = styles[_i], style_block += style + ":" + this.search_field.css(style) + ";";
                return div = $("<div />", {
                    style: style_block
...
}.call(this), // Generated by CoffeeScript 1.8.0
function() {
    var __indexOf = [].indexOf || function(item) {
        for (var i = 0, l = this.length; l > i; i++) if (i in this && this[i] === item) return i;
        return -1;
    };
    angular.module("localytics.directives", []), angular.module("localytics.directives").directive("chosen", [ "$timeout", function($timeout) {
        ...
        snakeCase = function(input) {
            ...
            link: function(scope, element, attr, ngModel) {
                ...
                angular.forEach(attr, function(value, key) {
                    ...
                    return element.addClass("loading").attr("disabled", !0).trigger("chosen:updated");
                }, stopLoading = function() {
                    return element.removeClass("loading").attr("disabled", !1).trigger("chosen:updated");
                ...
                valuesExpr = match[7], scope.$watchCollection(valuesExpr, function(newVal) {
                    var timer;
...
//# sourceMappingURL=glmultiselect.libs.min.js.map